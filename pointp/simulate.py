...
class Process1D(Process, ABC):
    ...
    def plot_func(self, tk: np.ndarray, t_max: float, **kwargs) -> go.Figure:
...
class Process2D(Process):
    ...
    def simulate(
        self, xbounds: tuple[float, float], ybounds: tuple[float, float]
    ) -> np.ndarray:
        """Return points generated by a simulation of the process on [t_min, t_max]."""
    ...
    def plot_func(
        ...
        **kwargs
        ...
        return plt.point_process_fig_2D(xk, yk, self.intensity, x_bounds, y_bounds,
                                        **kwargs)
...
class Homogeneous2D(Process2D):
    ...
    def simulate(
        ...
        num_pts = poisson.rvs(area)
        xk = x_bounds[0] + x_bounds[1] * np.random.rand(num_pts)
        yk = y_bounds[0] + y_bounds[1] * np.random.rand(num_pts)
